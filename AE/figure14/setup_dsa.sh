...
usage() {
    ...
    usage: $script [-d device (dsa0/iax1/..) ] [-w num wqs] [-q wq id] [-m wq mode (d or s)] [-s wq sz] [-e num eng] [-g grpID] [-b bind or not? (0 or 1)] [-x xfer size][-f BOF(0 or 1)]
		configures wqs
...
configure() {
	...
	0)
		...
		do
			if [[ $dname == dsa* ]] ; then 
				accel-config config-wq $dname/wq$did.$q --group-id=$grp_id --block-on-fault=$block_on_fault --mode=$mode --priority=10 --wq-size=$size --type=user --name=app$i --driver-name=user --max-transfer-size=$max_transfer_size --max-batch-size=$each_batch_size
			else
				accel-config config-wq $dname/wq$did.$q --group-id=$grp_id --block-on-fault=$block_on_fault --mode=$mode --priority=10 --wq-size=$size --type=user --name=app$i --driver-name=user
			fi
...
do_options() {
	...
	while getopts d:w:m:e:g:s:b:c:q:x:f: flag
	do
	    case "${flag}" in
		d)
			dname=${OPTARG}
		...
		q)
			...
			;;
		x)
		    max_transfer_size=${OPTARG}
			;;
		f)  
			block_on_fault=${OPTARG}
			;;
		:)
			echo 1
			usage >&2
	...
	then
		...
		then
			...
			size=$(( wq_size / num_wq ))
			if [[ $dname == dsa* ]]; then 
				batch_size=`cat /sys/bus/dsa/devices/$dname/max_batch_size`
				each_batch_size=$(( batch_size / num_wq ))
			fi
		else
			size=$wsz
		fi
